...
function makeLeafletHtml(pins: AnyPin[], isRestaurant: boolean, centerLat: number, centerLng: number): string {
  const markersJs = pins.map((p) => {
    ...
    const starsHtml = stars > 0 ? `<span style="font-size:10px;position:absolute;top:-4px;right:-4px;background:#E8C547;border-radius:8px;padding:0 3px;color:#000;font-weight:bold;">${'⭐'.repeat(Math.min(stars, 3))}</span>` : '';
